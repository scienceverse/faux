#' Get sample parameters from a data table
#'
#' Generates a table of the correlations and means of numeric columns in a data frame. If data was generated by `sim_design` and has a "design" attribute, between, within, dv and id are retrieved from that, unless overridden (use between = 0 to remove all between-subject factors)
#'
...
#' @examples
#' sample_params(iris, "Species")
#' @export
sample_params <- function(data, between = NULL, within = NULL, 
  ...
  if (length(within) && !is.null(dv) && !is.null(id) &&
    ...
    data <- long2wide(data, within, between, dv, id, sep)
    #data$id <- NULL
  }
  ...
  if (length(numvars) > 1) {
    ...
    if (is.null(within)) {
      ...
      stats[within] <- stats$var
    } else {
      # split var into columns
      splits <- strsplit(as.character(stats$var), split = sep, fixed = TRUE)
      stats[within] <- t(as.data.frame(splits))
    }
    
    var_order <- c(between, within, numvars, "n", "mean", "sd")
  } else {
    stats <- descriptives
...
#' [get_params] has been superceeded in favour of [sample_params], which gives the data in the same format as [sim_params].
#' 
#' Generates a table of the correlations and means of numeric columns in a data frame. If data was generated by \code{sim_design} and has a "design" attribute, between, within, dv and id are retrieved from that, unless overridden (use between = 0 to remove all between-subject factors)
#'
#' @param data the existing tbl
#' @param between a vector of column names for between-subject factors
#' @param within a vector of column names for within-subject factors (if data is long)
...
get_params <- function(data, between = NULL, within = NULL, 
                       dv = NULL, id = NULL, digits = 2) {
  lifecycle::deprecate_warn("1.2.4", "get_params()", "sample_params()")
...
#' @rdname sample_params
#' @export
check_sim_stats <- sample_params




#' Get simulation parameters from a data table
#'
#' Generates a table of the correlations and means of numeric columns in a data frame, if data was generated by [sim_design] and has a `design` attribute
#'
#' @param data the existing tbl
#' 
#' @export
...
#' @return a tbl of correlations, means and sds
#' @examples
#' dat <- sim_design(c(2, 2), c(2, 2), mu = 1:16, sd = 2)
#' sim_params(dat)
#' sample_params(dat)
sim_params <- function(data) {
  if ("design" %in% names(attributes(data))) {
    # get parameters from design
    design <- get_design(data)
  } else {
    warning("No design data was found, so returning sample parameters")
    params <- sample_params(data = data)
    return(params)
  }
